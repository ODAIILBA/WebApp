...
app.get('/admin/security/users-roles', async (c) => {
  ...
  try {
    ...
    const users = await db.db.prepare(`
      SELECT u.*, GROUP_CONCAT(r.role_display_name) as roles_list
      FROM users u
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ${roles.results.map((role: any) => `
                      <div class="border rounded-lg p-4 hover:shadow-md transition-shadow">
                        ...
                        <div class="flex gap-2">
                          <button class="flex-1 px-3 py-1.5 bg-gray-100 text-gray-700 rounded text-sm hover:bg-gray-200">
                            <i class="fas fa-edit mr-1"></i>Bearbeiten
                          </button>
                          <button class="px-3 py-1.5 bg-blue-100 text-blue-700 rounded text-sm hover:bg-blue-200">
